...
public final class DiagnosticsCommand extends CommandBase {
    ...
    public String getUsage(ICommandSender sender) {
        return "/cps <start [10|30|60]|stop|status|report>";
    }
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if (args.length == 0 || "status".equalsIgnoreCase(args[0])) {
            sender.sendMessage(new TextComponentString(DiagnosticsManager.isCapturing(Endpoint.SERVER)
                    ? "[CPS] 服务端正在采集，剩余 " + DiagnosticsManager.getRemainingMillis() / 1000.0 + " 秒"
                    : "[CPS] 服务端当前没有采集任务"));
            return;
        }
        switch (args[0].toLowerCase()) {
            case "start" -> {
                ...
                boolean started = DiagnosticsManager.startServer(seconds);
                sender.sendMessage(new TextComponentString(started
                        ? "[CPS] 已开始服务端采集，持续 " + seconds + " 秒"
                        : "[CPS] 已有采集任务正在运行"));
            }
            case "stop" -> {
